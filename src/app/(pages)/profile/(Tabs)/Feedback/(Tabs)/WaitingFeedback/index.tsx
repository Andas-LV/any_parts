...
export default function WaitingFeedback() {
    ...
    const {unRatedOrders, getUnratedOrders, isLoading, error} = useFeedbacksStore()
        ...
        <div className={styles.wrapper}>
            {unRatedOrdersEmpty ? (
                <div className={styles.noContentWrapper}>
                    ...
                    <p>
                        Спасибо что помогаете другим покупателям сделать выбор
                    </p>
                        ...
                        {unRatedOrders.map((order, index) => (
                            <Card key={index} className={styles.card}>
                                <CardHeader className={styles.cardHeader}>
                                    {order.image &&
                                        <img src={order.image} alt={order.name} className={styles.image}/>
                                    }
            ...
            {isFeedbackOpen && <CreateFeedback feedbackType={"Новый"} onClose={() => setIsFeedbackOpen(false)} itemId={itemId} />}
        </div>
    )
}