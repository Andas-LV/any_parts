...
export default function FilteredItemsHeader() {
    ...
    const priceLabel = `от ${priceRange[0]}₸ до ${priceRange[1]}₸`;
    ...
    if (highRated) {
        chips.push(
            <div key="highRated" className={styles.chip}>
                Рейтинг: 4.5+
                <Icons.PrimaryClose className={styles.closeBtn} onClick={removeHighRated} />
            </div>
        );
    }
    ...
    selectedBrands.forEach((brand) => {
        chips.push(
            <div key={`brand-${brand}`} className={styles.chip}>
                {brand}
                <Icons.PrimaryClose className={styles.closeBtn} onClick={() => removeBrand(brand)} />
            </div>
        );
    });
    ...
    selectedTypes.forEach((type) => {
        chips.push(
            <div key={`type-${type}`} className={styles.chip}>
                {type}
                <Icons.PrimaryClose className={styles.closeBtn} onClick={() => removeType(type)} />
            </div>
        );
    });
    ...
    selectedColors.forEach((color) => {
        chips.push(
            <div key={`color-${color}`} className={styles.chip}>
                {color}
                <Icons.PrimaryClose className={styles.closeBtn} onClick={() => removeColor(color)} />
            </div>
        );
    });
    ...
    selectedManufacturers.forEach((man) => {
        chips.push(
            <div key={`man-${man}`} className={styles.chip}>
                {man}
                <Icons.PrimaryClose className={styles.closeBtn} onClick={() => removeManufacturer(man)} />
            </div>
        );
    });
    ...
    if (isPriceChanged) {
        chips.push(
            <div key="priceRange" className={styles.chip}>
                {priceLabel}
                <Icons.PrimaryClose className={styles.closeBtn} onClick={resetPriceRange} />
            </div>
        );
    }
    ...
    return (
        <div className={styles.wrapper}>
            {chips.length > 0 && <div className={styles.chipsContainer}>
                {chips}
            </div>}
        </div>
    );
