...
export default function SearchPage({ params }: { params: Promise<{ search: string }> }) {
    ...
    const selectedSortName = itemSearchedSorts.find(sort => sort.value === selectedSort)?.name || "Выберите сортировку";
                            ...
                            <div className={styles.filter}>
                                ...
                                <DropdownMenu onOpenChange={setIsFilterOpen}>
                                    <DropdownMenuTrigger className={styles.sortSelect}>
                                        ...
                                        <Icons.ArrowDown className={`${styles.arrowIcon} ${isFilterOpen ? styles.rotated : ""}`} />
                                    </DropdownMenuTrigger>
                                    <DropdownMenuContent>
                                        {itemSearchedSorts.map(sort => (
                                            <DropdownMenuItem key={sort.value} onSelect={() => setSelectedSort(sort.value)}>
                                                {sort.name}
                                            </DropdownMenuItem>
                                        ))}
                                    </DropdownMenuContent>
