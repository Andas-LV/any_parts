...
const CreateReq = ({ onClose }: { onClose: () => void }) => {
    ...
    const {
        ...
        defaultValues: { country: countryCodes[0].value, name: "", BIK: "", account: "", fullName: "" },
    });
    ...
    const handleCountrySelect = (country: any) => {
        setSelectedCountry(country); // Сохраняем полный объект страны для отображения
        setValue("country", country.value, { shouldValidate: true }); // Сохраняем только value в форме
        ...
        <ModalsLayout title="Добавить реквизиты" back onClose={onClose}>
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-start gap-5 w-[550px] max-w-[800px]">
                ...
                <input
                    ...
                    {...register("name")}
                    className="w-full p-3 bg-[#F2F2F2] rounded-lg text-base focus:outline-none"
                />
                ...
                <input
                    ...
                    className="w-full p-3 bg-[#F2F2F2] rounded-lg text-base focus:outline-none"
                />
                {errors.BIK && <p className="text-red-500 text-sm">{errors.BIK.message}</p>}
