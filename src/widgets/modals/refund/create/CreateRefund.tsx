...
export default function CreateRefund({onClose, onNext}: { onClose: () => void, onNext: () => void }) {
    const { itemInfo, setCurrentItem } = useItemsStore();
    const [search, setSearch] = useState("");
    const [selectedItemId, setSelectedItemId] = useState<string>("");

    const filteredItems = useMemo(() => {
        return itemInfo?.filter(item =>
    ...
    const handleRefund = () => {
        const selectedItem = itemInfo?.find(item => item.id.toString() === selectedItemId) || null;
        if (selectedItem) {
